...
try:
    ...
    console.print(f"[red]❌ Gagal load file ABI JSON: {str(e)}[/red]")
    exit()
...
def warning_membership():
    with Live(refresh_per_second=2) as live:
        for i in range(20):  
            if i % 2 == 0:
                panel = Panel.fit(
                    "[bold red]🔺❗ JOIN MEMBERSHIP untuk bot lainnya! ❗🔺\n\n"
                    "[bold yellow]👉 https://t.me/AirdropFamilyIDN 👈[/bold yellow]",
...
def approve_token_lp(token, spender, amount, sender, privkey):
    ...
    allowance = contract.functions.allowance(sender, spender).call()
    if allowance >= amount:
        console.print(f"[green]✅ Allowance cukup untuk {token[-4:]}[/green]")
        return
    tx = contract.functions.approve(spender, amount).build_transaction({
        "chainId": CHAIN_ID,
        "from": sender,
    ...
    receipt = web3.eth.wait_for_transaction_receipt(tx_hash)
    if receipt.status == 1:
        console.print(f"[green]✅ Approve sukses: {tx_hash.hex()}[/green]")
    else:
        console.print(f"[red]❌ Approve gagal: {tx_hash.hex()}[/red]")
...
def approve_token_stake(token_addr, owner, spender, amount, key):
    ...
    console.print(f"[green]✅ Approve {token_addr[-4:]} sukses[/green]")
...
def stake_r2usd_to_sr2usd(sender, privkey):
    ...
    if balance < amount:
        console.print(f"[yellow]⚠️ R2USD terlalu kecil ({balance / 1e6})[/yellow]")
        return
...
def stake_wbtc(sender, privkey):
    ...
    balance = token.functions.balanceOf(sender).call()
    if balance < amount:
        console.print(f"[yellow]⚠️ WBTC balance terlalu kecil ({balance / 1e8})[/yellow]")
        return None 
...
def main():
    try:
        ...
        console.print("[red]❌ File pkevm.txt not found![/red]")
        return
...
if __name__ == "__main__":
    while True:
        try:
            ...
            main()
            console.print("[bold green]✅ Semua proses selesai. Menunggu 24 jam sebelum ulang...[/bold green]")
            console.print("🕒 Waktu berikutnya: " + time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time() + 86400)))
            time.sleep(86400)
        except Exception as e:
            console.print(f"[red]❌ Terjadi error saat loop: {e}[/red]")
            console.print("[yellow]⏳ Coba lagi setelah 60 detik...[/yellow]")
            time.sleep(60)
